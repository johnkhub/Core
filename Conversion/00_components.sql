...
AS
BEGIN
	--INSERT INTO  [BCM_CORE].[Core].[public].[asset] (asset_id, asset_type_code, adm_path, grap_path) VALUES (@asset_id,@asset_type_id, @adm_path, @grap_path)
...
CREATE PROCEDURE [dbo].[sp_link_asset_to_external]
(
	@asset_id uniqueidentifier, 
...
AS
BEGIN
	--INSERT INTO  [BCM_CORE].[Core].[public].[asset_link] (asset_id, external_id, external_id_type) VALUES (@asset_id,@external_id,@external_id_type)
...
CREATE PROCEDURE [dbo].[sp_link_asset_to_facility]
(
	@asset_id uniqueidentifier, 
...
CREATE PROCEDURE [dbo].[sp_process_money_field]
(
	@asset_id varchar(40), 
...
CREATE PROCEDURE [dbo].[sp_process_attribute]
	@asset_id varchar(40), 
	@batch_id uniqueidentifier, 
	@transaction_type varchar(10), 
	@field varchar(20), 
...
/*
	insert into table "Transaction" 
		("TransactionId", "TransactionTypeCode", "AssetId", "BatchId", "SubmitDate", "EffectiveDate", "Reason", "Field", "Amount", "DataType") 
	values 
		(NEWID, @transaction_type, @component_id, @batch_id, NOW, @effective_date, "Initial load", @field, 'T_MONEY')  
*/
	DECLARE @uuid AS uniqueidentifier = NEWID()
